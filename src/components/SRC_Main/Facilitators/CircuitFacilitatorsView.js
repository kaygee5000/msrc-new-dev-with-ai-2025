...
export default function CircuitFacilitatorsView({ filterParams }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Group all data by school for drill-down
      const allData = [
...
<FacilitatorManagement filterParams={filterParams} />
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Circuit Facilitator Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All Circuit Facilitators</Typography>
          <DataDisplayTable data={facilitators} title="Circuit Facilitators" />
          
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>Circuit Facilitator Attendance & Punctuality</Typography>
          <DataDisplayTable data={attendance} title="Circuit Facilitator Attendance" />

          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>Circuit Facilitator Lesson Data</Typography>
          <DataDisplayTable data={lessonData} title="Circuit Facilitator Lessons" />
        </Box>
      )}
      ...
      {schoolsData.map((school) => (
        <Accordion key={school.school_id} sx={{ mb: 1 }}>
          ...
          <AccordionDetails>
            {/* Reusing the SchoolFacilitatorsView for detailed school data */}
            <SchoolFacilitatorsView filterParams={{ school_id: school.school_id, year: filterParams.year, term: filterParams.term, week: filterParams.week }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
