...
// Mapping of enrollment metrics to their display properties
const ENROLLMENT_METRICS = {
  'Total Enrollment': {
  ...
  'Male Students': {
    icon: <MaleIcon sx={{ fontSize: 56 }} />,
    category: 'male',
    color: 'info'
  },
  'Female Students': {
    icon: <FemaleIcon sx={{ fontSize: 56 }} />,
    category: 'female',
    color: 'secondary'
  },
  'New Enrollments': {
...
// Helper function to get display properties for enrollment metric
const getMetricDisplay = (metricName, data) => {
  const metric = ENROLLMENT_METRICS[metricName] || {
    icon: <HelpOutlineIcon sx={{ fontSize: 56 }} />,
    category: 'other',
    color: 'default'
  };
  
  // Determine enrollment status based on data
  let status = 'stable';
  ...
  if (!data || Object.keys(data).length === 0) {
    ...
    // Check for enrollment indicators in the data
    const hasHighEnrollment = Object.values(data).some(value => 
      (typeof value === 'number' && value > 100) ||
      (typeof value === 'string' && (
        value.toLowerCase().includes('high') ||
        value.toLowerCase().includes('increasing') ||
        value.toLowerCase().includes('growing')
      ))
    );
    
    const hasLowEnrollment = Object.values(data).some(value => 
      (typeof value === 'number' && value < 50) ||
      (typeof value === 'string' && (
        value.toLowerCase().includes('low') ||
        value.toLowerCase().includes('decreasing') ||
        value.toLowerCase().includes('declining')
      ))
    );
    
    if (hasHighEnrollment && !hasLowEnrollment) {
      status = 'growing';
      statusColor = 'success';
      trendIcon = <TrendingUpIcon sx={{ fontSize: 20 }} />;
    } else if (hasHighEnrollment && hasLowEnrollment) {
      status = 'stable';
      statusColor = 'warning';
      trendIcon = <TrendingUpIcon sx={{ fontSize: 20 }} />;
    } else if (hasLowEnrollment) {
      status = 'declining';
      statusColor = 'error';
...
const getSummaryStats = (data) => {
  if (!data || data.length === 0) return { enrolled: 0, total: 0, percent: 0, status: 'No Data' };
  
  const total = data.length;
  const enrolled = data.filter(item => {
    ...
    return dataKeys.some(key => {
      ...
      return (typeof value === 'number' && value > 0) ||
             (typeof value === 'string' && (
               value.toLowerCase().includes('enrolled') ||
  ...
  const percent = Math.round((enrolled / total) * 100);
  
  let status = 'Low Enrollment';
  if (percent === 100) status = 'Fully Enrolled';
  else if (percent >= 80) status = 'High Enrollment';
  else if (percent >= 60) status = 'Good Enrollment';
  else if (percent >= 40) status = 'Moderate Enrollment';
  
  return { enrolled, total, percent, status };
};
...
const transformDataToMetrics = (rawData) => {
  ...
  // Create sample metrics based on common enrollment indicators
  // In a real implementation, this would map actual database fields to metrics
  const metrics = [
    { name: 'Total Enrollment', data: rawData[0] },
    { name: 'Male Students', data: rawData[0] },
    { name: 'Female Students', data: rawData[0] },
    { name: 'New Enrollments', data: rawData[0] },
    { name: 'Enrollment Trend', data: rawData[0] },
    { name: 'Grade Distribution', data: rawData[0] },
    { name: 'Early Childhood', data: rawData[0] },
  ];
  
  return metrics;
};

export default function SchoolStudentEnrollmentView({ filterParams }) {
        ...
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {stats.enrolled} of {stats.total} enrollment metrics active
            </Typography>
          </Box>
          <Box sx={{ textAlign: 'right' }}>
            ...
            <Typography variant="caption" color="text.secondary">
              {stats.percent}% enrollment activity
            </Typography>
          </Box>
        </Stack>
      </Paper>
              ...
              <Grid key={idx} size={{ xs: 12, sm: 6, md: 4 }}>
                ...
                >
                  <CardContent sx={{ textAlign: 'center' }}>
                    ...
                    <Chip
                      label={
                        display.status === 'growing' ? 'Growing' :
                        display.status === 'stable' ? 'Stable' :
                        display.status === 'declining' ? 'Declining' : 'No Data'
                      }
