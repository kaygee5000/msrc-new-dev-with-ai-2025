...
// Import the enhanced CircuitStudentEnrollmentView for drill-down
import CircuitStudentEnrollmentView from './CircuitStudentEnrollmentView';
...
// Mapping of student enrollment metrics to their display properties
const ENROLLMENT_METRICS = {
  'Total Enrollment': {
    icon: <GroupIcon sx={{ fontSize: 56 }} />,
    category: 'total_enrollment',
    color: 'primary'
  },
  'Boys Enrollment': {
    icon: <BoyIcon sx={{ fontSize: 56 }} />,
    category: 'boys_enrollment',
    color: 'info'
  },
  'Girls Enrollment': {
    icon: <GirlIcon sx={{ fontSize: 56 }} />,
    category: 'girls_enrollment',
    color: 'secondary'
  },
  'New Enrollment': {
...
const getEnrollmentDisplay = (metricType, data) => {
  const metric = ENROLLMENT_METRICS[metricType] || {
...
const getSummaryStats = (data) => {
  if (!data || data.length === 0) return { totalStudents: 0, status: 'No Data' };
  
  let totalStudents = 0;
  data.forEach(item => {
    totalStudents += item.total_enrollment || 0;
  });

  let status = 'Enrollment Data Available';
  if (totalStudents === 0) status = 'No Enrollment Data';
  
  return { totalStudents, status };
};
...
const transformDataToMetrics = (rawData) => {
  ...
  rawData.forEach(item => {
    aggregatedData.total_enrollment += item.total_enrollment || 0;
    aggregatedData.boys_enrollment += item.boys_enrollment || 0;
    aggregatedData.girls_enrollment += item.girls_enrollment || 0;
    aggregatedData.new_enrollment += item.new_enrollment || 0;
  });

  const metricTypes = Object.keys(ENROLLMENT_METRICS).map(name => {
    const categoryKey = ENROLLMENT_METRICS[name].category;
...
export default function DistrictStudentEnrollmentView({ filterParams, loadOnDemand = false, reportTitle = 'Student Enrollment' }) {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>District Student Enrollment Summary</Typography>
          ...
          <Box sx={{ textAlign: 'right' }}>
            <Typography 
              variant="h4" 
              color={
                stats.status === 'Enrollment Data Available' ? 'success.main' : 'warning.main'
              }
            >
              {stats.status}
            </Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All District Student Enrollment Data</Typography>
      ...
      {circuitsData.map((circuit) => (
        <Accordion key={circuit.circuit_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the CircuitStudentEnrollmentView for detailed circuit data */}
            <CircuitStudentEnrollmentView filterParams={{ circuit_id: circuit.circuit_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
