...
import DistrictPupilPerformanceView from './DistrictPupilPerformanceView';
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Region Pupil Performance Summary</Typography>
            <Typography variant="body2" color="text.secondary">
              {stats.excellent} excellent, {stats.good} good, {stats.average} average, {stats.poor} poor across {districtsData.length} districts
            </Typography>
          </Box>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the DistrictPupilPerformanceView for detailed district data */}
            <DistrictPupilPerformanceView filterParams={{ district_id: district.district_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
