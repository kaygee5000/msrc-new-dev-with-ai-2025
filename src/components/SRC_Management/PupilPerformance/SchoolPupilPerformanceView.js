...
const PERFORMANCE_METRICS = {
  ...
  'Academic Achievement': {
    icon: <EmojiEventsIcon sx={{ fontSize: 56 }} />,
    category: 'achievement',
    color: 'success'
  },
  'Grade Distribution': {
    icon: <GradeIcon sx={{ fontSize: 56 }} />,
    category: 'grades',
    color: 'info'
  },
  'Performance Trend': {
...
// Helper function to get display properties for a performance metric
const getMetricDisplay = (metricName, data) => {
  const metric = PERFORMANCE_METRICS[metricName] || {
    icon: <HelpOutlineIcon sx={{ fontSize: 56 }} />,
    category: 'other',
    color: 'default'
  };
  
  // Determine performance status based on data
  let status = 'good';
  let statusColor = metric.color;
  let trendIcon = <TrendingFlatIcon sx={{ fontSize: 20 }} />;
  
  if (!data || Object.keys(data).length === 0) {
    status = 'no_data';
    statusColor = 'default';
  } else {
    // Check for performance indicators in the data
    const hasGoodPerformance = Object.values(data).some(value => 
    ...
    const hasPoorPerformance = Object.values(data).some(value => 
    ...
    if (hasGoodPerformance && !hasPoorPerformance) {
      status = 'excellent';
      statusColor = 'success';
      trendIcon = <TrendingUpIcon sx={{ fontSize: 20 }} />;
    } else if (hasGoodPerformance && hasPoorPerformance) {
      status = 'average';
      statusColor = 'warning';
      trendIcon = <TrendingFlatIcon sx={{ fontSize: 20 }} />;
...
const getSummaryStats = (data) => {
  if (!data || data.length === 0) return { performing: 0, total: 0, percent: 0, status: 'No Data' };
  
  const total = data.length;
  const performing = data.filter(item => {
    // Check if the item shows good performance
  ...
  const percent = Math.round((performing / total) * 100);
  
  let status = 'Needs Improvement';
  if (percent === 100) status = 'Excellent';
  else if (percent >= 80) status = 'Very Good';
  else if (percent >= 60) status = 'Good';
  else if (percent >= 40) status = 'Fair';
  
  return { performing, total, percent, status };
};
...
const transformDataToMetrics = (rawData) => {
  ...
  // Create sample metrics based on common performance indicators
  // In a real implementation, this would map actual database fields to metrics
  const metrics = [
    { name: 'Overall Performance', data: rawData[0] },
    { name: 'Academic Achievement', data: rawData[0] },
    { name: 'Grade Distribution', data: rawData[0] },
    { name: 'Performance Trend', data: rawData[0] },
    { name: 'Subject Performance', data: rawData[0] },
    { name: 'Class Performance', data: rawData[0] },
  ];
  
  return metrics;
};

export default function SchoolPupilPerformanceView({ filterParams }) {
        ...
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {stats.performing} of {stats.total} metrics showing good performance
            </Typography>
          </Box>
          <Box sx={{ textAlign: 'right' }}>
            ...
            <Typography variant="caption" color="text.secondary">
              {stats.percent}% performance rating
            </Typography>
          </Box>
        </Stack>
      </Paper>
