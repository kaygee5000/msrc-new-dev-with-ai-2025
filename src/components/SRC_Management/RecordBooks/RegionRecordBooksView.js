...
export default function RegionRecordBooksView({ filterParams }) {
  ...
  if (!data || data.length === 0) return (
    <Paper elevation={0} sx={{ p: 2, mt: 1 }}>
      <Typography variant="body1">No record books data available for schools in this region.</Typography>
    </Paper>
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" gutterBottom>
        Region Record Books Data - {districtsData.length} Districts
      </Typography>
      
      {/* Summary of all record books data */}
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>All Region Record Books Data</Typography>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            <Typography variant="subtitle2" gutterBottom>District Record Books Summary</Typography>
