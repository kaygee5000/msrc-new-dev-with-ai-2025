...
// Import the enhanced SchoolRecordBooksView for drill-down
import SchoolRecordBooksView from './SchoolRecordBooksView';
...
export default function CircuitRecordBooksView({ filterParams }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      recordBooksData.forEach(item => {
        ...
        schoolMap.get(item.school_id).recordBooks.push(item);
      });
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Circuit Record Books Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All Circuit Record Books Data</Typography>
      ...
      {schoolsData.map((school) => (
        <Accordion key={school.school_id} sx={{ mb: 1 }}>
          ...
          <AccordionDetails>
            {/* Reusing the SchoolRecordBooksView for detailed school data */}
            <SchoolRecordBooksView filterParams={{ school_id: school.school_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
