...
// Import the enhanced DistrictSupportGrantsView for drill-down
import DistrictSupportGrantsView from './DistrictSupportGrantsView';
...
export default function RegionSupportGrantsView({ filterParams, loadOnDemand = false, reportTitle = 'Support Grants' }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      supportGrantsData.forEach(item => {
        ...
        district.supportGrants.push(item);
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Region Support Grants Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All Region Support Grants Data</Typography>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the DistrictSupportGrantsView for detailed district data */}
            <DistrictSupportGrantsView filterParams={{ district_id: district.district_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
