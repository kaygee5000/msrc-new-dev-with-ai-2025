...
export default function DistrictTextbooksView({ filterParams, loadOnDemand = false, reportTitle = 'Textbooks' }) {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>District Textbook Availability</Typography>
            <Typography variant="body2" color="text.secondary">
              {stats.available} of {stats.total} textbook types available across {circuitsData.length} circuits
            </Typography>
          </Box>
      ...
      {circuitsData.map((circuit) => (
        <Accordion key={circuit.circuit_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the CircuitTextbooksView for detailed circuit data */}
            <CircuitTextbooksView filterParams={{ circuit_id: circuit.circuit_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
