...
import DistrictSecurityView from './DistrictSecurityView';
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Region Security Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All Region Security Data</Typography>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the DistrictSecurityView for detailed district data */}
            <DistrictSecurityView filterParams={{ district_id: district.district_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
