...
export default function DistrictSecurityView({ filterParams }) {
  ...
  if (!data || data.length === 0) return (
    <Paper elevation={0} sx={{ p: 2, mt: 1 }}>
      <Typography variant="body1">No security data available for schools in this district.</Typography>
    </Paper>
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" gutterBottom>
        District Security Data - {circuitsData.length} Circuits
      </Typography>
      
      {/* Summary of all security data */}
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>All District Security Data</Typography>
      ...
      {circuitsData.map((circuit) => (
        <Accordion key={circuit.circuit_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            <Typography variant="subtitle2" gutterBottom>Circuit Security Summary</Typography>
            ...
            <Box sx={{ mt: 2 }}>
              {circuit.schools.map((school) => (
                <Accordion key={school.school_id} sx={{ mb: 1 }}>
                  ...
                  <AccordionDetails>
                    <DataDisplayTable data={school.security} title={`${school.school_name} Security`} />
                  </AccordionDetails>
                </Accordion>
              ))}
            </Box>
