...
import CircuitSecurityView from './CircuitSecurityView';
...
export default function DistrictSecurityView({ filterParams, loadOnDemand = false, reportTitle = 'Security' }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      securityData.forEach(item => {
        ...
        circuit.schools.get(item.school_id).security.push(item);
      });
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>District Security Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All District Security Data</Typography>
      ...
      {circuitsData.map((circuit) => (
        <Accordion key={circuit.circuit_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            {/* Reusing the CircuitSecurityView for detailed circuit data */}
            <CircuitSecurityView filterParams={{ circuit_id: circuit.circuit_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
