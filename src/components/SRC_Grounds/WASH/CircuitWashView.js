...
export default function CircuitWashView({ filterParams }) {
  ...
  if (!data || data.length === 0) return (
    <Paper elevation={0} sx={{ p: 2, mt: 1 }}>
      <Typography variant="body1">No WASH data available for schools in this circuit.</Typography>
    </Paper>
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" gutterBottom>
        Circuit WASH Data - {schoolsData.length} Schools
      </Typography>
      
      {/* Summary of all WASH data */}
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>All Circuit WASH Data</Typography>
