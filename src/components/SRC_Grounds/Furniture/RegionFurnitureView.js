...
export default function RegionFurnitureView({ filterParams }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      furnitureData.forEach(item => {
        ...
        districtMap.get(item.district_id).furniture.push(item);
      });
      
      setDistrictsData(Array.from(districtMap.values()));
    } catch(e) { 
  ...
  if (!data || data.length === 0) return (
    <Paper elevation={0} sx={{ p: 2, mt: 1 }}>
      <Typography variant="body1">No furniture data available for schools in this region.</Typography>
    </Paper>
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" gutterBottom>
        Region Furniture Data - {districtsData.length} Districts
      </Typography>
      
      {/* Summary of all furniture data */}
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>All Region Furniture Data</Typography>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 1 }}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography>{district.district_name}</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <DataDisplayTable data={district.furniture} title={`${district.district_name} Furniture`} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
