...
// Import the enhanced SchoolFurnitureView for drill-down
import SchoolFurnitureView from './SchoolFurnitureView';
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Stack direction="row" spacing={3} alignItems="center" justifyContent="space-between">
          <Box>
            <Typography variant="h6" gutterBottom>Circuit Furniture Summary</Typography>
      ...
      {viewMode === 'card' ? (
        ...
        <Box>
          <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>All Circuit Furniture Data</Typography>
      ...
      {schoolsData.map((school) => (
        <Accordion key={school.school_id} sx={{ mb: 1 }}>
          ...
          <AccordionDetails>
            {/* Reusing the SchoolFurnitureView for detailed school data */}
            <SchoolFurnitureView filterParams={{ school_id: school.school_id, year: filterParams.year, term: filterParams.term }} />
          </AccordionDetails>
        </Accordion>
      ))}
    </Box>
