...
export default function RegionCommunityInvolvementView({ filterParams }) {
  ...
  if (!data || data.length === 0) return (
    <Paper elevation={0} sx={{ p: 2, mt: 1 }}>
      <Typography variant="body1">No community involvement data available for schools in this region.</Typography>
    </Paper>
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Typography variant="h6" gutterBottom>
        Region Community Involvement Data - {districtsData.length} Districts
      </Typography>
      
      {/* Summary of all community involvement data */}
      <Paper elevation={1} sx={{ p: 2, mb: 3 }}>
        <Typography variant="subtitle1" gutterBottom>All Region Community Involvement Data</Typography>
      ...
      {districtsData.map((district) => (
        <Accordion key={district.district_id} sx={{ mb: 2 }}>
          ...
          <AccordionDetails>
            <Typography variant="subtitle2" gutterBottom>District Community Involvement Summary</Typography>
                    ...
                    <Box sx={{ mt: 1 }}>
                      {circuit.schools.map((school) => (
                        <Accordion key={school.school_id} sx={{ mb: 1 }}>
                          ...
                          <AccordionDetails>
                            <DataDisplayTable data={school.involvement} title={`${school.school_name} Community Involvement`} />
                          </AccordionDetails>
                        </Accordion>
                      ))}
                    </Box>
